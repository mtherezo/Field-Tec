...
export default function FormAtendimentoScreen() {
  ...
  // (Lógica de estados, refs, useEffect, etc., continua a mesma)
  ...
  useEffect(() => {
    if (isEditMode) {
      const carregarDadosParaEdicao = async () => {
        const atendimentosSalvos = await getAtendimentos();
        const atendimentoParaEditar = atendimentosSalvos.find(at => at.id === atendimentoId);
  ...
  const handleSave = async () => {
    ...
    const atendimentosAtuais = await getAtendimentos();
    let novaLista = [];
    const dadosDoFormulario = { numeroChamado, numeroLogicoTerminal, solicitacao, diagnosticoSolucao, causaReal, status, detalhePendencia: isPendente ? detalhePendencia : '', dataInicio: dataInicio.toISOString(), dataFim: dataFim ? dataFim.toISOString() : null };
    if (isEditMode) {
      const atendimentoAtualizado: Atendimento = { id: atendimentoId, ...dadosDoFormulario };
      novaLista = atendimentosAtuais.map(at => at.id === atendimentoId ? atendimentoAtualizado : at);
    } else {
      const novoAtendimento: Atendimento = { id: Date.now().toString(), ...dadosDoFormulario };
      novaLista = [...atendimentosAtuais, novoAtendimento];
    }
  ...
  // ✅ 2. O RETORNO AGORA É ENVOLVIDO PELA SafeAreaView
  return (
